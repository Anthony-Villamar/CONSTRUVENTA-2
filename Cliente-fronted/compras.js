...
function agregarProducto(codigo, nombre, precio, peso, stockDisponible) {
  const cantidadInput = document.getElementById("cantidad_" + codigo);
  const cantidad = parseInt(cantidadInput.value);

  if (cantidad > stockDisponible) {
    alert(`No puedes agregar ${cantidad} unidades. Solo hay ${stockDisponible} en stock.`);
    return;
  }
  ...
  if (item) {
    if (item.cantidad + cantidad > stockDisponible) {
      alert(`No puedes agregar más de ${stockDisponible} unidades en total para ${nombre}.`);
      return;
    }
